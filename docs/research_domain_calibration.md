...
$$
\mathcal{S}^{d-1} = \{ v \in \mathbb{R}^d \mid \|v\|_2 = 1 \} \tag{1}
$$
...
* Vector đặc trưng thu được từ miền nguồn $S$:
  $$
  v_s^{(i)} \approx z_i \tag{2}
  $$
* Vector đặc trưng thu được từ miền đích $T$:
  $$
  v_t^{(i)} \approx z_i + B + \epsilon_i \tag{3}
  $$
...
$$
\text{Sim}(v_s^{(i)}, v_t^{(i)})
= \frac{v_s^{(i)} \cdot v_t^{(i)}}{\|v_s^{(i)}\| \|v_t^{(i)}\|}
\approx \frac{z_i \cdot (z_i + B + \epsilon_i)}{\|z_i\| \|z_i + B + \epsilon_i\|}
\ll 1.0
\tag{4}
$$
...
v'^{(j)}_t
  &= v_t^{(j)} + \alpha \Delta v \\
  &\approx z_j + B + \epsilon_j + \alpha(z_k - z_k - B) \\
  &= z_j + (1-\alpha)B + \alpha z_k + \epsilon_j
\end{aligned}
\tag{11}
$$
...
$$
\begin{aligned}
\text{Sim}\!\left(v'^{(j)}_t,\, v_s^{(k)}\right)
  &\approx \text{Sim}\!\left(z_j + (1-\alpha)B + \alpha z_k,\; z_k\right) \\
  &= \frac{z_j \cdot z_k + (1-\alpha)B \cdot z_k + \alpha \|z_k\|^2}
         {\|z_j + (1-\alpha)B + \alpha z_k\|_2 \, \|z_k\|_2}
\end{aligned}
\tag{12}
$$
...
\begin{aligned}
\text{Sim}\!\left(v'^{(j)}_t,\, v_s^{(k)}\right)
  &\approx \frac{\alpha}{\sqrt{\|z_j + (1-\alpha)B\|_2^2 + \alpha^2}}
\end{aligned}
\tag{13}
$$
